...
from torchjd.aggregation import GradVacWeighting, IMTLGWeighting, MeanWeighting
...
gramian_pairs = [
    (GramianWeightedAggregator(CRMOGMWeighting(IMTLGWeighting())), m)
    for m in typical_matrices + scaled_matrices
]
...
def test_reset_restores_first_step_behavior() -> None:
    """
    Use ``IMTLGWeighting`` so the weights actually depend on the input — with
    ...
    J = randn_((3, 8))
    G = J @ J.T
    W = CRMOGMWeighting(IMTLGWeighting(), alpha=0.5)
    first = W(G)
    W(G)
    W.reset()
...
def test_alpha_zero_reduces_to_bare_weighting() -> None:
    ...
    G = J @ J.T
    bare = IMTLGWeighting()
    smoothed = CRMOGMWeighting(IMTLGWeighting(), alpha=0.0)
...
def test_alpha_one_freezes_weights() -> None:
    ...
    J = randn_((3, 8))
    G = J @ J.T
    W = CRMOGMWeighting(IMTLGWeighting(), alpha=1.0)
    first = W(G)
...
def test_ema_is_applied() -> None:
    ...
    bare = IMTLGWeighting()
    smoothed = CRMOGMWeighting(IMTLGWeighting(), alpha=alpha)
...
def test_initial_weights_used_as_lambda_0() -> None:
    ...
    bare = IMTLGWeighting()
    W = CRMOGMWeighting(IMTLGWeighting(), alpha=alpha, initial_weights=initial)
